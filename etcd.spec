...
%define gobuild(o:) go build -mod vendor -buildmode pie -tags=rpm_crashtraceback -ldflags "${LDFLAGS:-} -B 0x$(head -c20 /dev/urandom|od -An -tx1|tr -d ' \\n') -extldflags '-Wl,-z,relro,-z,now'" -a -v -x %{?**};
...
install -m 0755 -vd %{_builddir}/%{name}-%{version}/_build/bin
export GOPATH="%{_builddir}/%{name}-%{version}/_build:${GOPATH:+${GOPATH}:}/%{gopath}"
install -m 0755 -vd "$(dirname %{_builddir}/%{name}-%{version}/_build/src/%{goipath})"
ln -fs %{_builddir}/%{name}-%{version} %{_builddir}/%{name}-%{version}/_build/src/%{goipath}
cd %{_builddir}/%{name}-%{version}/_build/src/%{goipath}
...
%build
export GOPATH=%{_builddir}/%{name}-%{version}/_build:$(pwd)/Godeps/_workspace:%{gopath}
%gobuild -o %{_builddir}/%{name}-%{version}/bin/etcd %{goipath}
%gobuild -o %{_builddir}/%{name}-%{version}/bin/etcdctl %{goipath}/etcdctl
